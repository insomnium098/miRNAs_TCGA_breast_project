...
datos_mirnas <- read.csv("DATOS_P_VALUES_KAPLAN.csv",
                         header = TRUE)

mirnas_cool <- filter(datos_mirnas, p.val < 0.01 )

mirnas_loop <- as.character(mirnas_cool$microrna)
#mirnas_loop <- mirnas_loop[1]
...
###########
colnames(dataset_Super_chingon)[1] <- mirna

dataset_Super_chingon <- dataset_Super_chingon[,c(1,2,4,5,6,7)]
colnames(dataset_Super_chingon)[2] <- paste0(mirna,"_valor_expresion")
#Si el datasset no existe, se crea

if (!exists("dataset_final_chingon")){
  dataset_final_chingon <- dataset_Super_chingon
} else{
  #Si el dataset existe, se une 
  temp_dataset <- dataset_Super_chingon[,c(1,2)]
  dataset_final_chingon <- cbind(dataset_final_chingon,temp_dataset)
  rm(temp_dataset)
...
datos_clinicos_filtrados <- datos_clinicos[rownames(dataset_final_chingon),]

datos_clinicos_chingon <- datos_clinicos_filtrados[,c("age_at_diagnosis" , "tumor_stage")]


dataset_final_super <- merge(dataset_final_chingon, datos_clinicos_chingon, by = "row.names")
...
write.csv(dataset_final_super,"DATOS_PROCESADOS.csv")
